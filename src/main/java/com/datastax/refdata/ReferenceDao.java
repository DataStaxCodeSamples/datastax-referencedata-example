...
public class ReferenceDao {
	...
	private static String tableNameMetaData = keyspaceName
			+ ".exchange_metadata";

	private static final String INSERT_INTO_HISTORIC = "Insert into "
			+ tableNameHistoric
			+ " (exchange,symbol,date,open,high,low,close,volume,adj_close) values (?,?,?,?,?,?,?,?,?);";
	private static final String INSERT_INTO_DIVIDENDS = "Insert into "
			+ tableNameDividends
			+ " (exchange,symbol,date,dividend) values (?,?,?,?);";
	private static final String INSERT_INTO_METADATA = "Insert into "
			+ tableNameMetaData
	...
	public void insertHistoricData(List<HistoricData> list) throws Exception {
		...
		if (mostRecent != null) {
			BoundStatement boundMetaDataStmt = new BoundStatement(
					this.insertStmtMetaData);
			boundMetaDataStmt.setString("exchange", mostRecent.getExchange());
			boundMetaDataStmt.setString("symbol", mostRecent.getSymbol());
			boundMetaDataStmt
					.setDate("last_updated_date", mostRecent.getDate());
